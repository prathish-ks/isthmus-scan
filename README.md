...
NanoClaw agents run in containers, but a handful of settings decide how much of your real machine they can touch if one gets compromised or prompt-injected. `isthmus-scan` checks six of them:
...
If it's an [Isthmus](https://github.com/prathish-ks/isthmus) checkout, it adds a seventh check — **kernel liveness** — confirming the Go trust-kernel is actually running and enforcing, not just installed.
